...
| P8 | Hub, navigation-helper, and TV timer mutations use the shell snackbar. Other in-app results still use `Toast` (destinations, EPG dialog, video, profiles, backup, screenshot, picon sync, hub loading, missing stream player). | `*Destination`, `EpgEventDialogSession`, `VideoOverlayController`, `BaseActivity`, `HubDestination` | Remaining toasts move into ViewModel UI state in C2 |
