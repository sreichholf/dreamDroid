...
| near-zero-java-kotlin-helpers | [#318](https://github.com/sreichholf/dreamDroid/pull/318) | open | Port tiny helpers to Kotlin (`Python`, `BundleHelper`, `ProfileChangedListener`, `SimpleResult`/`Message`/`Tag`/`Volume`/`PowerState`/`SleepTimer`/`Remote`, `DreamDroidAttributionPresenter`). Keep dialogs / VideoOverlay / Leanback / RemoteViews / HttpURLConnection. |
