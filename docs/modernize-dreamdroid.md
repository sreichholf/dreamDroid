...
| P6 | Routes are string constants with `{arg}` templates; profile-edit arguments are carried on `PhoneNavHostState` instead of the route. `NavigationHelper` maps drawer menu ids to routes. | `ui/nav/PhoneNavRoutes.kt`, `PhoneNavHostState.kt`, `NavigationHelper.kt` | Not covered by any rule |
| P7 | TV MultiEPG and TV settings are separate activities (`MultiEpgActivity`, `PreferenceActivity`) opened by intent extras. Launch goes through a `TabbedNavigationActivity` trampoline that picks phone vs TV at runtime. | `tv/activities/`, `activities/TabbedNavigationActivity.kt`, manifest | Not covered by any rule |
...
| Step | Fixes | Change | Proof |
| --- | --- | --- | --- |
| D1 | P6 | Type-safe routes: `@Serializable` route classes (Kotlin serialization plugin) replace `PhoneNavRoutes` strings. Profile-edit and other arguments move into their routes; `PhoneNavHostState` keeps only shell state. Drawer items map to route objects, not menu ids. `dreamdroid://` deep links are declared on routes. | Navigation tests per argumented route; deep-link test. |
| D2 | P7 | One TV activity: `MultiEpgActivity` and `PreferenceActivity` become destinations of a TV NavHost in `tv.activities.MainActivity`. Replace the `TabbedNavigationActivity` trampoline with launcher categories on the two activities (`LAUNCHER` on phone, `LEANBACK_LAUNCHER` on TV). Keep an `activity-alias` named `.activities.TabbedNavigationActivity` only if pinned shortcuts from 1.15 need it (check before deleting). | TV instrumented tests open MultiEPG and settings as routes; launcher intents resolve on phone and TV images. |
