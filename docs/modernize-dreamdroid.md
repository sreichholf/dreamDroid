...
| http-async-epgnownext | [#218](https://github.com/sreichholf/dreamDroid/pull/218) | open | Phase 2.2f: Hub now/next via `lifecycleScope` + suspend `EnigmaClient.getEpgNowNext()`; drop `GetEpgNowNextTask`. |
...
- Phase 2.2f (this PR): Hub now/next coroutine load.
...
| 2f | HTTP / async — Hub now/next | `ServiceListPageFragment` → `lifecycleScope` + suspend `EnigmaClient.getEpgNowNext()`; delete `GetEpgNowNextTask`. **[#218](https://github.com/sreichholf/dreamDroid/pull/218).** |
