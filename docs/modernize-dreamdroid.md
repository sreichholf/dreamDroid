...
| http-async-timers | [#216](https://github.com/sreichholf/dreamDroid/pull/216) | open | Phase 2.2d: Timer list via `lifecycleScope` + suspend `EnigmaClient.getTimers()`; drop `GetTimerListTask`; keep generation guards. |
...
- Phase 2.2d (this PR): Timer list coroutine load.
...
| 2d | HTTP / async — Timers | `TimerListFragment` → `lifecycleScope` + suspend `EnigmaClient.getTimers()`; delete `GetTimerListTask`; keep generation guards. **[#216](https://github.com/sreichholf/dreamDroid/pull/216).** |
