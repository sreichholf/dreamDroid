...
| drop-jvm-interop-annotations | [#372](https://github.com/sreichholf/dreamDroid/pull/372) | Drop `@JvmStatic` / `@JvmOverloads` / `@JvmField` left from Java→Kotlin ports now that `app/src` has no Java. Keep `@JvmField` on `Profile` so explicit getX/setX do not clash. |
