...
| Operator usertests | **Phone verified** (2026-09-19). Phone drawer MultiEPG is gone (nested from hub toolbar/overflow and list EPG Timeline). Tablet + real Android TV / box still open. File bugs; no drive-by refactors. Then a bugfix pass, one PR per fix. In-tree TV gate: `ComposeTvHubStubTest` / `Chrome` / `ServiceRow` / `MovieRow` / `TvMultiEpgScreenTest`. Box: cold-start Compose hub (not Leanback); D-pad headers↔rows; Settings Reload/Preferences/Profile (Profile is a list: switch/add/edit/delete); drawer **MultiEPG** → bouquet cards → D-pad grid (or bouquet service-grid clock) → detail stream/timer; bouquet → stream; movie location lazy load → file stream; profile switch reloads. |
