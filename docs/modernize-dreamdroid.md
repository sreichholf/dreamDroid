...
| Item | Notes |
| --- | --- |
| ViewModels | Phone rows 1–19 have landed (#491, #492, #493, #496, #498). Remaining: service EPG and hub now-playing (rows 19a/19b), TV hosts (row 20), share and setup (21), dialog routes (22), and player (23). Order, rules, and the per-screen PR list: [ViewModels](#viewmodels) below. Do not fold a row into unrelated work. |
...
Status (2026-09-23): rows 1–19 are done. Every phone NavHost destination in those rows gets its state from a `ViewModel`, `PhoneNavHostState` is activity-scoped on a `SavedStateHandle`, and the hub pages share the hub entry's scope. Rows 19a, 19b, and 20–23 are open.
...
| 22 | Dialog routes | `SleepTimer`, `SendMessage`, and `Power` only when they own a load or a session. `About` stays composition. Visibility flags stay in the dialog composable. | Open |
