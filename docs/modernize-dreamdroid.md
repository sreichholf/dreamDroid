...
| near-zero-java-kotlin-helpers | — | open | Port tiny helpers to Kotlin (`Python`, `BundleHelper`, `ProfileChangedListener`, `SimpleResult`/`Message`/`Tag`/`Volume`/`PowerState`/`SleepTimer`/`Remote`, `DreamDroidAttributionPresenter`). Keep dialogs / VideoOverlay / Leanback / RemoteViews / HttpURLConnection. |
...
Wave 3 (operator choice): convert remaining **non-Compose phone UIs** to Compose + Kotlin, **one PR per screen**. Checklist in Appendix G. **Wave 3 phone screens complete on `main` through #206** (plus CI #204). Phone Phase 2.7 (Fragment shells → Kotlin Compose destinations) **2.7b–i merged** ([#306](https://github.com/sreichholf/dreamDroid/pull/306)–[#315](https://github.com/sreichholf/dreamDroid/pull/315)). **#316 merged**. **#317 merged**. **This PR:** Kotlin-port tiny helpers (Signal/DeviceInfo SAX edge/dead menus + Base64). **Next:** further near-zero Java / workarounds + Phase **3.1c-iv** TV / Phase 4–5.
...
- Phone NavHost Device Info through Hub are **Kotlin Compose destinations**; Phase **2.7b–i** **merged** ([#306](https://github.com/sreichholf/dreamDroid/pull/306)–[#315](https://github.com/sreichholf/dreamDroid/pull/315); decision [#304](https://github.com/sreichholf/dreamDroid/pull/304)). **#316 merged**. **#317 merged**. **This PR:** Kotlin-port tiny helpers (Signal/DeviceInfo SAX edge/dead menus + Base64).
...
- **#316 merged**. **#317 merged**. **This PR:** Kotlin-port tiny helpers (orphan `Signal`, unused DeviceInfo handler/SAX/hash keys, dead menus; Base64 → `android.util.Base64`). Keep dialogs / VideoOverlay / Leanback / RemoteViews / HttpURLConnection Enigma2.
...
**Safe next after Phase 0 / 3.1c focus dive landed:** Phase 3 cards/prefs (done). Broader Appendix H phone NavHost/widgets/VLC/state **merged**. Phone Phase **2.7b–h** destinations **merged**; **2.7i merged** [#315](https://github.com/sreichholf/dreamDroid/pull/315). **#316 merged**. **#317 merged**. **This PR:** Kotlin-port tiny helpers. **Next:** further near-zero Java exceptions + Phase **3.1c-iv** full Compose TV hub (option C) / Phase 4–5.
...
| Bases | `fragment/abs/BaseFragment.java` | Titles / multi-pane hooks for host + remaining non-NavHost Fragments |
...
| 2.7i | Chassis cleanup: drop unused `BaseHttp*` / recycler bases / `HttpFragmentHelper` / `NestedFragmentDestination`; `NavExtras.DATA`; TV `LOADER_DEFAULT_ID = 0` | **merged** [#315](https://github.com/sreichholf/dreamDroid/pull/315). No dialog policy reopen; no VideoActivity fold |
...
**Phase 2.7i merged** [#315](https://github.com/sreichholf/dreamDroid/pull/315). **#316 merged**. **#317 merged**. **This PR:** Kotlin-port tiny helpers (Signal/DeviceInfo SAX edge/dead menus + Base64). **Next:** further near-zero Java exceptions + **Phase 3.1c-iv** (full Compose TV hub / option C), then Phase 4–5.
...
- Phase **2.7i** chassis cleanup **merged** [#315](https://github.com/sreichholf/dreamDroid/pull/315). Prefer finishing near-zero Java dead-weight before **3.1c-iv-b+** hub Compose (unless the operator waives the gate).
