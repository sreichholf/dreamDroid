...
| http-async-simpleresult | [#224](https://github.com/sreichholf/dreamDroid/pull/224) | open | Phase 2.2k: Mutations via `lifecycleScope` + `SimpleResultLoad`; drop `SimpleResultTask` (HttpFragmentHelper, NavigationHelper, ShareActivity). |
...
- Phase 2.2k (this PR): SimpleResult mutations coroutine load (`SimpleResultTask`).
...
| 2k | HTTP / async — SimpleResult mutations | `HttpFragmentHelper` + `NavigationHelper` + `ShareActivity` → `lifecycleScope` + `SimpleResultLoad`; delete `SimpleResultTask`. **[#224](https://github.com/sreichholf/dreamDroid/pull/224).** |
| 2 | HTTP / async stack (program) | Replace `HttpURLConnection` + executor/`Loader` with Kotlin coroutines + typed `EnigmaClient` everywhere; keep OkHttp 3.14.9 for Picasso until a dedicated bump. Follow-ons after 2k: power/volume/sleeptimer tasks, Loader chassis. |
