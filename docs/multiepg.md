...
| **Detail** | Full-screen TV Surface + `EpgDetailBody` / `EpgDetailScreen(showActions=false)`. Stream (5-arg `getStreamServiceIntent` + `startStreamIntent`), Set timer (`launchSimpleResultLoad` + toast), IMDb. No Edit timer / Similar; no `EpgEventDialogSession`. |
