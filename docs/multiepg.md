...
`DatabaseHelper` is a read-only importer of leftover `dreamdroid` profile rows. It does not create or write `events` / `services`. **Do not** revive those writers. New cache goes through **Room** (`AppDatabase`).
